...
namespace OptimizerBeta3.Areas.TransactionTablePages.Controllers
{
    ...
    public class DeliveryChallanController : Controller
    {
        ...
        public DeliveryChallanController(ApplicationDbContext db)
        {
            _db = db;
        ...
        public async Task<IActionResult> Index(DateTime? fromDate, DateTime? toDate)
        {
            ...
            return View(await _db.DeliveryChallans.OrderByDescending(x => x.Id).Where(x => x.DCDt >= effectStartDate && x.DCDt <= effectEndDate && x.InvoiceTo == "LS").ToListAsync());

            //return View(await _db.DeliveryChallans.ToListAsync());
        }
        ...
        public async Task<IActionResult> CreatePost(string save)
        {
            ...
            int nCity = unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().FKCity;
            int nArea = unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().FKArea;
            int nState = unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().FKState;
            int nPincode = unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().FKPincode;
            int nFKCompany = unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().FKCompanyInfo;

            deliveryChallanVM.DeliveryChallan.City = lookUpMaster.Where(x => x.Id == nCity).FirstOrDefault().Description;
            deliveryChallanVM.DeliveryChallan.GSTNo = _db.companyInfos.Where(x => x.Id == nFKCompany).FirstOrDefault().GSTNumber;
            deliveryChallanVM.DeliveryChallan.Address= unitmaster.Where(x => x.Id == deliveryChallanVM.DeliveryChallan.FKToUnit).FirstOrDefault().Address1;
            deliveryChallanVM.DeliveryChallan.Area = lookUpMaster.Where(x => x.Id == nArea).FirstOrDefault().Description;
            deliveryChallanVM.DeliveryChallan.InvoiceTo = "LS";
        ...
        //GET - CREATE
        public async Task<IActionResult> DCDetailsCreate()
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DCDetailsCreate(DeliveryChallanDetailViewModel deliveryChallanDetailVM)
        {
