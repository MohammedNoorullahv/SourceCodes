...
namespace OptimizerBeta3.Areas.TransactionTablePages.Controllers
{
    ...
    public class LGInwardController : Controller
    {
        ...
        public LGInwardController(ApplicationDbContext db, IWebHostEnvironment hostingEnvironment)
        {
            _db = db;
        ...
        public async Task<IActionResult> Index(DateTime? fromDate, DateTime? toDate)
        {
            ...
            return View(await _db.inwards.OrderByDescending(x => x.Id).Where(x => x.InwardDt >= effectStartDate && x.InwardDt <= effectEndDate && x.FLAM == "L").ToListAsync());
        }
        ...
        public IActionResult IndexFilter(DateTime fromDate, DateTime toDate)
        {
            ...
            return RedirectToAction("Index", "LGInward", new { fromDate = fromDate, toDate = toDate });
        }
        ...
        public async Task<IActionResult> Create()
        {
            ...
            string sqlQuery = $"EXEC SLI_Filters @mAction='SELSUPPINLGINW', @mControllerName='{controllerName}', @mActionMethod='{actionName}'";
        ...
        public async Task<IActionResult> CreatePost(string Save, IFormFile formFile)
        {
            ...
            else if (Save == "Save & Insert Dtl")
            {
                ...
                return RedirectToAction("CreateInwardDetailForScanning", "LGInward", new { Id = inward.Id });
            }
            else
            {
                var inward = await _db.inwards.Where(x => x.InwardNo == inwardVM.Inward.InwardNo).FirstOrDefaultAsync();
                ...
                string sqlQuery = $"EXEC SLI_Inwards @mAction='SELLGINWARD', @mInwardNo='{inward.InwardNo}'";
                var cmd = _db.Database.GetDbConnection().CreateCommand();
                cmd.CommandText = sqlQuery;
                _db.Database.OpenConnection();
                ...
                return RedirectToAction("LoadUploadedInwardeDetail", "LGInward", new { Id = inward.Id });
        ...
        public async Task<IActionResult> CreateInwardDetailForReScanning(int Id)
        {
            ...
            return RedirectToAction("CreateInwardDetailForScanning", "LGInward", new { Id = Id });
        }
        ...
        public async Task<IActionResult> InwardDetailScannedCodes(MdlTempArticalArrivalEANCode model)
        {
            ...
            if (sScanningMode != "Updates")
            {
                ...
                string sqlQuery = $"EXEC SLI_Inwards @mAction='SELLGINWARD', @mInwardNo='{sInwardNo}'";
                var cmd = _db.Database.GetDbConnection().CreateCommand();
                cmd.CommandText = sqlQuery;
                _db.Database.OpenConnection();
