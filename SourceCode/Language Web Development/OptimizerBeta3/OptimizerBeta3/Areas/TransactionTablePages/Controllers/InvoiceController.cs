...
namespace OptimizerBeta3.Areas.TransactionTablePages.Controllers
{
    ...
    public class InvoiceController : Controller
    {
        ...
        public InvoiceController(ApplicationDbContext db, IWebHostEnvironment hostingEnvironment)
        {
            _db = db;
        ...
        public async Task<IActionResult> Index(DateTime? fromDate, DateTime? toDate)
        {
            ...
            return View(await _db.Invoices.OrderByDescending(x => x.Id).Where(x => x.InvoiceDt >= effectStartDate && x.InvoiceDt <= effectEndDate && x.InvoiceTo == "PS").ToListAsync());
        }
        ...
        public async Task<IActionResult> CreatePost(string Save, IFormFile formFile)
        {
            ...
            //InvoiceVM.Invoice.ToLocation = 0;
            ...
            else if (Save == "Save & Insert Dtl")
            {
                ...
                return RedirectToAction("CreateInvoiceDetailForScanning", "Invoice", new { Id = invoice.Id });
            }
            else
            {
                var invoice = await _db.Invoices.Where(x => x.InvoiceNo == InvoiceVM.Invoice.InvoiceNo).FirstOrDefaultAsync();
                ...
                return RedirectToAction("LoadTempInvoiceDtls", "Invoice", new { Id = invoice.Id });
            }
        }
        ...
        public async Task<IActionResult> GenerateDC(int? id)
        {
            ...
            string sqlQuery = $"EXEC SLI_DeliveryChallan @mAction='SELINVFORDCPS', @mId='{id}'";
            var cmd = _db.Database.GetDbConnection().CreateCommand();
            cmd.CommandText = sqlQuery;
            _db.Database.OpenConnection();
        ...
        public async Task<IActionResult> CreateInvoiceDetailForReScanning(int Id)
        {
            //TODO: To remove the Wrong Barcodes in the Temporary List
            var scndlist = await _db.TempInvoiceDtlEANCodes.Where(x => x.IPAddress == ipaddress && x.IsMatching == false).ToListAsync();
            _db.TempInvoiceDtlEANCodes.RemoveRange(scndlist);
            await _db.SaveChangesAsync();

            sScanningMode = "Re Entries";
            return RedirectToAction("CreateInvoiceDetailForScanning", "Invoice", new { Id = Id });
        }
        ...
        public async Task<IActionResult> InvoiceDetailScannedCodes(TempInvoiceDtlEANCode model)
        {
            ...
            if (sScanningMode != "Updates")
            {
                ...
                var scannedlist1 = Request.Form["cntr_list1"];
            ...
            return RedirectToAction("LoadTempInvoiceDtls", "Invoice", new { Id = InvoicefromDb.Id });
        ...
        public async Task<IActionResult> LoadTempInvoiceDtls(int Id)
        {
            ...
            //tempInv.ValueinINR = "1st sentence";
            //sentence.Two = "2nd sentence";

            var invoiceSum = await _db.TempInvoiceDtls.Where(x => x.FKInvoiceNo == Id).ToListAsync();
            decimal nQuantity = 0;
            decimal dValue, dDiscountValue, dGrossValue, dGSTValue, dNettValue;
            dValue = 0; dDiscountValue = 0; dGrossValue = 0; dGSTValue = 0; dNettValue = 0;
            foreach (var item in invoiceSum)
            {
                nQuantity += item.Quantity;
                dValue += item.Value;
                dDiscountValue += item.DiscountValue;
                dGrossValue += item.GrossValue;
                dGSTValue += item.GSTTotalValue;
                dNettValue += item.ItemNettValue;
            }

            //int nRoundOffNettValue = Math.Round(dNettValue,0);
            
            Invoice tempInv = new Invoice();
            tempInv.Id = Id;
            tempInv.ItemsValue = dValue;
            tempInv.ValueinINR = dValue;
            tempInv.ItemsDiscountValue = dDiscountValue;
            tempInv.ItemsGrossValue = dGrossValue;
            tempInv.GSTValues = dGSTValue;
            decimal dDifference = Convert.ToDecimal(Convert.ToInt32(dNettValue) - dNettValue);
            if (dDifference < 0)
            {
                tempInv.RoundoffPlus = 0;
                tempInv.RoundoffMinus = dDifference;
            }
            else
            {
                tempInv.RoundoffPlus = dDifference;
                tempInv.RoundoffMinus = 0;
            }
            tempInv.ItemsNettValue = dNettValue + tempInv.RoundoffPlus - tempInv.RoundoffMinus;
            ViewBag.TempInv = tempInv;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> InsertInvoiceDetail(string InvoiceId, string InvDiscPercentage, string InvDiscValue, string OthersPlus,string OthersMinus,string InvValue)
        {
            int Id = Convert.ToInt32(InvoiceId); //Convert.ToInt32(TempData["InvoiceId"]);
            ...
            var invoiceSum = await _db.InvoiceDetails.Where(x => x.FKInvoiceNo == Id).ToListAsync();
            decimal nQuantity = 0;
            decimal dValue, dDiscountValue, dGrossValue, dGSTValue, dNettValue;
            dValue = 0; dDiscountValue = 0; dGrossValue = 0; dGSTValue = 0; dNettValue = 0;
            foreach (var item in invoiceSum)
            ...
            var InvfromDb = await _db.Invoices.Where(x => x.Id == Id).FirstAsync();
            InvfromDb.Quantity = nQuantity;
            InvfromDb.ItemsValue = dValue;
            InvfromDb.ValueinINR = dValue;
            InvfromDb.GSTValues = dGSTValue;
            InvfromDb.ItemsGrossValue = dGrossValue;
            InvfromDb.ItemsDiscountValue = dDiscountValue;
            InvfromDb.DiscountPercentage = Convert.ToDecimal(InvDiscPercentage);
            InvfromDb.DiscountValue = Convert.ToDecimal(InvDiscValue);
            InvfromDb.OtherExpensesPlus = Convert.ToDecimal(OthersPlus);
            InvfromDb.OtherExpensesMinus = Convert.ToDecimal(OthersMinus);
            InvfromDb.NettValue = Convert.ToDecimal(InvValue);
            InvfromDb.IsEntryCompleted = true;
            decimal dDifference = Convert.ToDecimal(Convert.ToInt32(dNettValue) - dNettValue);
            if (dDifference < 0)
            {
                InvfromDb.RoundoffPlus = 0;
                InvfromDb.RoundoffMinus = dDifference;
            }
            else
            {
                InvfromDb.RoundoffPlus = dDifference;
                InvfromDb.RoundoffMinus = 0;
            }
            InvfromDb.ItemsNettValue = dNettValue + InvfromDb.RoundoffPlus - InvfromDb.RoundoffMinus;
            _db.SaveChanges();
        ...
        #endregion
        //GET
        #region "INVOICE DETAIL FROM NOTEPAD/EXCEL"
        //GET
        public async Task<IActionResult> CreateInvoiceDetailForDataFetching(int Id)
        {
            //var inv = await _db.Invoices.FindAsync(nFKInvoice);
            if (Id == 0)
            {
                Id = Convert.ToInt32(TempData["FKInvoice"]);
            }
            var inv = await _db.Invoices.FindAsync(Id);
            TempData["Invoice"] = inv;
        ...
        //POST
        [HttpPost, ActionName("CreateInvoiceDetailForDataFetchingPost")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateInvoiceDetailForDataFetchingPost(IFormFile formFile)
        {
            try
            {
                int nFKInvoice = Convert.ToInt32(Request.Form["InvoiceId"]);
                TempData["FKInvoice"] = nFKInvoice;

                var scndlist = _db.TempInvoiceDtlEANCodes.Where(x => x.IPAddress == ipaddress);
                _db.TempInvoiceDtlEANCodes.RemoveRange(scndlist);
                await _db.SaveChangesAsync();

                var tmpInvLst = _db.TempInvoiceDtls.Where(x => x.FKInvoiceNo == nFKInvoice);
                _db.TempInvoiceDtls.RemoveRange(tmpInvLst);
                await _db.SaveChangesAsync();
                ...
                else
                {
                    using (var stream = System.IO.File.Open(filePath, FileMode.Open, FileAccess.Read))
                    {
                        using (var reader = ExcelReaderFactory.CreateReader(stream))
                        {
                            ...
                            {
                                scannedlist.Add(reader.GetValue(0).ToString());
                                //if (res.Select(x => x.EANCode).ToList().Contains(reader.GetValue(0).ToString()))
                                //{
                                //    res.Where(x => x.EANCode == reader.GetValue(0).ToString()).First().Quantity++;
                                //}
                                //else
                                //{
                                //    res.Add(new MdlTempArticalArrivalEANCode
                                //    {
                                //        EANCode = reader.GetValue(0).ToString(),
                                //        Quantity = 1
                                //    });
                                //}
                            }
                ...
                //_db.TempArticalArrivalEANCodes.AddRange(res);
                //_db.SaveChanges();

                var InvoicefromDb = await _db.Invoices.FindAsync(nFKInvoice);
                string sInvoiceNo = InvoicefromDb.InvoiceNo;
                int nFKLocation = InvoicefromDb.FKLocation;
                int nFKUnit = InvoicefromDb.FKUnit;
                ...
                foreach (var EANCode in scannedlist)
                {
                    int nRowCnt = _db.stocks.Where(x => (x.EANCode == EANCode || x.StockNo == EANCode) && x.FKUnit == nFKUnit && x.FKLocation == nFKLocation && x.FKStatus == nFKStatus).ToList().Count;
                    if (nRowCnt == 0)
                    {
                        int nRowCnt1 = _db.TempInvoiceDtlEANCodes.Where(x => x.EANCode == EANCode).ToList().Count;
                        if (nRowCnt1 == 0)
                        {
                            model.IPAddress = ipaddress;
                            model.InvoiceNo = sInvoiceNo;
                        ...
                        {
                            var TempFromdb = await _db.TempInvoiceDtlEANCodes.Where(x => x.EANCode == EANCode).FirstOrDefaultAsync();
                            TempFromdb.Quantity = TempFromdb.Quantity + 1;
                            _db.SaveChanges();
                        }
                ...
                //ViewBag.TempInvoiceDtls = _db.TempInvoiceDtls.ToList();
                //ViewBag.TempEANCodeDtls = _db.TempInvoiceDtlEANCodes.Where(x => x.IsMatching == false).ToList();
                return RedirectToAction("LoadUploadedInvoiceDetail", "Invoice", new { Id = nFKInvoice });

            }
            catch (Exception)
            {
                TempData["ErrorMessage"] = "Uploaded file is not a valid file";
                //TempData.Keep("FKInvoice");
                int nFKInvoice = Convert.ToInt32(Request.Form["InvoiceId"]);
                return RedirectToAction("CreateInvoiceDetailForDataFetching", "Invoice", new { Id = nFKInvoice });
            }

        }


        public async Task<IActionResult> LoadUploadedInvoiceDetail(int Id)
        {
            //var inv = await _db.Invoices.FindAsync(nFKInvoice);
            var inv = await _db.Invoices.FindAsync(Id);
            TempData["Invoice"] = inv;
        ...
        #region "INVOICE DETAIL FROM NOTEPAD"
        //[HttpPost]
        //[ValidateAntiForgeryToken]
        public async Task<IActionResult> LoadFromNotepad(int Id)
        {
            var inv = await _db.Invoices.FindAsync(Id);
            TempData["Invoice"] = inv;
