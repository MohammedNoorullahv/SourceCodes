...
Public Class frmTommyContainerScanning
    ...
    Private Sub tbOuterCartonNoforViewing_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles tbOuterCartonNoforViewing.KeyPress
        ...
        If keyascii = 13 Then
            ...
            If Trim(tbOuterCartonNoforViewing.Text) = "" Then
                ...
                MsgBox("Invalid Outer CartonNo - Outer Carton Should not  be Left Blank")
                Exit Sub
            End If
            ...
            If Microsoft.VisualBasic.UCase(Trim(tbOuterCartonNoforViewing.Text)) = "NEW ROW" Then
                tbRowNumber.Text = Val(nLastRowNo) + 1
                tbOuterCartonNoforViewing.Clear()
                tbOuterCartonNoforViewing.Focus()
                Exit Sub
            End If

            Dim daSelPackingListInfo As New SqlDataAdapter("Select * from PackingDetail where Barcode = '" & Trim(tbOuterCartonNoforViewing.Text) & _
                                                           "' And InvoiceNo = '" & Trim(tbInvoiceNo.Text) & "' Order by cartonNo", sCnn)
            ...
            Else
                ...
                'Else
                Dim daSelPackingInfo As New SqlDataAdapter("Select * from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & _
                                                           "' And Barcode = '" & Trim(tbOuterCartonNoforViewing.Text) & _
                                                           "'", sCnn)
                ...
                tbTotalQty.Text = dsSelPackingListInfo.Tables(0).Rows(0).Item("Quantity")
    ...
    Private Sub tbOuterCartonNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles tbOuterCartonNo.KeyPress
        ...
        If keyascii = 13 Then
            ...
            If Trim(tbOuterCartonNoforViewing.Text) <> Trim(tbOuterCartonNo.Text) Then
                Me.AxWindowsMediaPlayer1.URL = "E:\pingwarb-7327.mp3"
                MsgBox("Outer Carton No Mismatch", MsgBoxStyle.Critical)
                Exit Sub
            End If

          
            'Dim daSelPackingInfo As New SqlDataAdapter("Select * from PackingInfo Where JobcardNo = '" & Trim(tbInvoiceNo.Text) & _
            ...
            'End If
            Dim sId As String
            Dim nSlNo, nSSlNo As Integer
            sId = System.Guid.NewGuid.ToString()

            Dim daSelSlNo As New SqlDataAdapter("Select IsNull(Max(CartonSlNo),0) As CartonSlNo from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & "'", sConstr)
            Dim dsSelSlNo As New DataSet
            daSelSlNo.Fill(dsSelSlNo)

            nSlNo = Val(dsSelSlNo.Tables(0).Rows(0).Item("CartonSlNo")) + 1

            Dim daSelSSlNo As New SqlDataAdapter("Select IsNull(Max(CartonSubSerialNo),0) As CartonSlNo from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & _
                                                 "' And RowNo = '" & Val(tbRowNumber.Text) & "'", sConstr)
            Dim dsSelSSlNo As New DataSet
            daSelSSlNo.Fill(dsSelSSlNo)

            nSSlNo = Val(dsSelSSlNo.Tables(0).Rows(0).Item("CartonSlNo")) + 1

            Dim daInsPackingInfo As New SqlDataAdapter("Insert Into ContainerLoadingDetails(ID,InvoiceNo,RowNo,Barcode,CartonSlNo,CartonSubSerialNo,LoadedOn) Values ('" & sId & _
                                                       "','" & Trim(tbInvoiceNo.Text) & "','" & Val(tbRowNumber.Text) & _
                                                       "','" & Trim(tbOuterCartonNo.Text) & "','" & nSlNo & "','" & nSSlNo & "','" & Format(Date.Now, "dd-MMM-yyyy hh:mm:ss") & "')", sCnn)
            ...
            If Val(tbBal2Scan.Text) <= 0 Then
                MsgBox("Total Scanning is Completed for this Container", MsgBoxStyle.Information)
                tbInvoiceNo.Clear()
                tbOuterCartonNo.Enabled = False
            End If
            'MsgBox("V:1")
    ...
    Private Sub Loadgrdinfo()
        ...
        Dim daSelPackingInfo As New SqlDataAdapter("Select * from PackingInfo Where JobcardNo = '" & Trim(tbInvoiceNo.Text) & "' And IsUpdatedinAP <> 1", sCnn)
        Dim dsSelPackingInfo As New DataSet
        daSelPackingInfo.Fill(dsSelPackingInfo)
        ...
        tbScanned.Text = nPackedCartons
        tbBal2Scan.Text = nTotalCartons - nPackedCartons
...
Ab:
    ...
    Private Sub tbInvoiceNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles tbInvoiceNo.KeyPress
        keyascii = AscW(e.KeyChar)
        If keyascii = 13 Then
            keyascii = 0

            If Trim(tbInvoiceNo.Text) = "" Then
                Me.AxWindowsMediaPlayer1.URL = "E:\pingwarb-7327.mp3"
                MsgBox("Invalid InvoiceNo -  InvoiceNo should not be left blank")
                Exit Sub
            End If

            Dim daSelPackingDetail As New SqlDataAdapter("Select * from PackingDetail where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & _
                                                         "' Order by PackingListNo,SalesOrderDetailid,CartonNo", sConstr)

            Dim dsSelPackingDetail As New DataSet
            daSelPackingDetail.Fill(dsSelPackingDetail)

            If dsSelPackingDetail.Tables(0).Rows.Count = 0 Then
                Me.AxWindowsMediaPlayer1.URL = "E:\pingwarb-7327.mp3"
                MsgBox("Invalid InvoiceNo")
                Exit Sub
            Else
                tbTotalBoxes.Text = Val(dsSelPackingDetail.Tables(0).Rows.Count)
                LoadPackedInfo()
                If Val(tbBal2Scan.Text) <= 0 Then
                    MsgBox("Total Scanning is Completed for this Container", MsgBoxStyle.Information)
                    tbInvoiceNo.Clear()
    ...
    Dim nLastRowNo As Integer

    Private Sub LoadLastRowNo()
        Dim daSelRowNo As New SqlDataAdapter("Select * from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & _
                                                     "' Order by CartonSlNo Desc", sConstr)
        Dim dsSelRowNo As New DataSet
        daSelRowNo.Fill(dsSelRowNo)

        If dsSelRowNo.Tables(0).Rows.Count = 0 Then
            tbRowNumber.Text = "1"
        Else
            nLastRowNo = Val(dsSelRowNo.Tables(0).Rows(0).Item("RowNo"))
            If Val(tbRowNumber.Text) = 0 Then
                tbRowNumber.Text = Val(dsSelRowNo.Tables(0).Rows(0).Item("RowNo"))
            End If
        End If
    End Sub

    Private Sub LoadRowBoxesCount()
        Dim daSelRowNo As New SqlDataAdapter("Select * from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & _
                                             "' And RowNo = '" & Val(tbRowNumber.Text) & _
                                             "' Order by CartonSlNo Desc", sConstr)
        Dim dsSelRowNo As New DataSet
        daSelRowNo.Fill(dsSelRowNo)
        ...
        If dsSelRowNo.Tables(0).Rows.Count = 0 Then
            tbRowTotal.Clear()
        Else
            tbRowTotal.Text = dsSelRowNo.Tables(0).Rows.Count
        End If
    End Sub

    Private Sub LoadPackedInfo()
        Dim daSelPackedCount As New SqlDataAdapter("Select * from ContainerLoadingDetails Where InvoiceNo = '" & Trim(tbInvoiceNo.Text) & "'", sConstr)
        Dim dsSelPackedCount As New DataSet
        daSelPackedCount.Fill(dsSelPackedCount)

        tbScanned.Text = dsSelPackedCount.Tables(0).Rows.Count
        tbBal2Scan.Text = Val(tbTotalBoxes.Text) - Val(tbScanned.Text)
    End Sub
    ...
    Private Sub tbRowNumber_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbRowNumber.Leave
        If Val(tbRowNumber.Text) > nLastRowNo + 1 Or Val(tbRowNumber.Text) < nLastRowNo - 1 Then
            MsgBox("Scanning cannot be done for More than +/- 1 Row(s)", MsgBoxStyle.Critical)
            tbRowNumber.Clear()
        End If
    End Sub

    Private Sub tbRowNumber_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tbRowNumber.TextChanged
